...
def index():
    ...
    created_at_dates = [date[0] for date in created_at_dates if date[0]]
    ...
    filter_params = {
        ...
        'created_at': request.args.getlist('created_at')
    }
    ...
    return render_template('books/index.html',
                           ...
                           created_at_dates=created_at_dates)
...
def create():
    ...
    try:
        ...
        db.session.add(book)
        db.session.commit()
        flash(f'Восхождение "{book.name}" было успешно добавлено!', 'success')

    except sa.exc.SQLAlchemyError:
        db.session.rollback()
        flash(f'При сохранении восхождения произошла ошибка', 'danger')
        genres = Genre.query.all()
...
def edit(book_id):
    ...
    if not book:
        flash(f'Такого восхождения не существует', 'warning')
        return redirect(url_for('books.index'))
...
def update(book_id):
    ...
    if not book:
        flash(f'Такой восхождения не существует', 'warning')
        return redirect(url_for('books.index'))
        ...
        for key in BOOK_PARAMS:
            if not getattr(book, key) or not book.short_desc or not book.genres:
                ...
                return render_template('books/update.html',
                                       genres=genres, book=book)
        db.session.commit()
        flash(f'Восхождение {book.name} было успешно изменена!', 'success')
...
def delete(book_id):
    ...
    try:
        ...
        db.session.commit()
        flash(f'Восхождение "{book.name}" успешно удалено', 'success')

    except sa.exc.SQLAlchemyError:
        flash(f'При удалении восхождения произошла ошибка', 'danger')
        db.session.rollback()
        return render_template('books/index.html')
    return redirect(url_for('books.index'))
...
def reviews(book_id):
    ...
    if not book_reviews.all():
        flash(f'У этого восхождения еще нет отзывов!', 'warning')
        return redirect(url_for('books.show', book_id=book_id))
    ...
    pagination = book_reviews.paginate(page=page, per_page=5)
