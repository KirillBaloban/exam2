...
def add_book(book_id):
    ...
    if not collection or not book:
        flash('Неверные данные о подборке или восхождении.', 'danger')
        return redirect(url_for('books.show', book_id=book_id))

    try:
        collection.books.append(book)
        db.session.commit()
        flash(f'Восхождение "{book.name}" добавлено в подборку "{collection.name}"!', 'success')
    except sa.exc.SQLAlchemyError:
        db.session.rollback()
        flash('Ошибка при добавлении восхождения в подборку.', 'danger')
